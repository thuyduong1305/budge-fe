...
const Income_Category = () => {
  ...
  const handleEdit = async id => {
    setEditMode(true);
    console.log(id);
    const income = await get(`income/${id}`);
    setItemEdit(income.data);
  ...
  const calculateTotalIncomeByCategory = (transactions, categoryName) => {
    let totalIncome = 0;

    transactions.forEach(transaction => {
      if (transaction.type === 'income' && transaction.name === categoryName) {
        totalIncome += transaction.money;
      }
    });
  ...
  data.forEach(category => {
    totalIncomeByCategory[category.name] = calculateTotalIncomeByCategory(
      dataPaymentHistory,
      category.name,
    );
  });
    ...
    <>
                    ...
                    <View style={styles.item}>
                      ...
                      <View>
                        ...
                        <View
                          ...
                          <Text>
                            {totalIncomeByCategory[item.name]
                              .toString()
                              .replace(/\B(?=(\d{3})+(?!\d))/g, ',') + ' đ'}
                          </Text>
                        </View>
      ...
      <View style={styles.add}>
        <TouchableOpacity onPress={() => setModalVisible(true)}>
          <Icon name="pluscircle" size={50} color={'#569657'}></Icon>
        </TouchableOpacity>
        <Modal
          animationType="slide"
          transparent={true}
